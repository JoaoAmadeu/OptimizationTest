...
namespace Terahard.Assessment.Test
{
    class TestContext : IContext
    {
        ...
        private static Object[] m_Objects = new Object[Core.Application.OBJECT_COUNT];
        ...
        public void Update( float deltaTime, int phase )
        {
            Vector2 pos = m_Objects[0].Position;
            switch (phase)
            {
                case 1:
                {
                    pos = new Vector2(
                        (int)Mathf.Lerp(pos.x, 0f, deltaTime / Core.Application.FRAMES),
                        (int)Mathf.Lerp(pos.y, 1000f, deltaTime / Core.Application.FRAMES)
                    );
                    break;
                }
                ...
                {
                    pos = new Vector2(
                        (int)Mathf.Lerp(pos.x, 1000f, deltaTime / Core.Application.FRAMES),
                        (int)Mathf.Lerp(pos.y, 1000f, deltaTime / Core.Application.FRAMES)
                    );
                    break;
                }
                ...
                case 3:
                {
                    pos = new Vector2(
                        (int)Mathf.Lerp(pos.x, 1000f, deltaTime / Core.Application.FRAMES),
                        (int)Mathf.Lerp(pos.y, 0f, deltaTime / Core.Application.FRAMES)
                    );
                    break;
                }
